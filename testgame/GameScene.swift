...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMoveToView(view: SKView) {
        ...
        if levels == "2"
        {
            verticalPipeGap = 300.0
        }
        if levels == "4"
        {
            verticalPipeGap = 290.0
        }
        if levels == "6"
        {
            verticalPipeGap = 250.0
        }
        ...
        bird.setScale(2.0)
        bird.position = CGPoint(x: self.frame.size.width * 0.36, y:self.frame.size.height * 0.6)
        bird.runAction(flap)
        ...
        scoreLabelNode.text = "Get the USB Drive Past \(levels) memory sticks" //String(score)
    ...
    func resetScene (){
        ...
        // Reset score
        score = 0
        scoreLabelNode.text = "Tap Screen to Move USB Drive Through Memory Maze" //String(score)
